...
class MusicLayerPlayer:
    ...
    def start(self) -> None:
        try:
            ...
            raise ImportError("pygame is required for S2 music playback. Install it with: pip install pygame") from exc
...
def send_ble_ui_summary(ser, detection: dict, intervention_result: dict, ppb_result: dict, enabled: bool) -> None:
    ...
    packet = f"U,{chewing},{cpm},{side},{stability},{ppb_tenths},{ppb_state}\n"
...
def run_terminal_loop(
    ...
    print(f"S2 running on {args.port}. Fast threshold={args.fast_cpm_threshold:.1f} CPM.")
...
def run_visual_loop(
    ...
    print(f"S2 visual interface running on {args.port}. Close the window or press Stop to quit.")
    ...
    s2_line, = ax_state.step([], [], where="post", color="#9467bd", linewidth=2, label="S2 state")
    ...
    status_text_2 = fig.text(0.02, 0.885, "S2: pause | stability: 0.00 | layers: none", fontsize=13)
    ...
    def update(_frame):
        ...
        status_text_2.set_text(
            f"S2: {intervention_result['intervention_state']} | "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="S2 spatial chewing music intervention system based on C3 detection.")

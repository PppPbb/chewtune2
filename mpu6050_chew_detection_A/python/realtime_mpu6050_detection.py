...
DEFAULT_GATE_MODEL_PATH = PROJECT_DIR / "mpu6050_chew_detection" / "models" / "non_chewing_gate.pkl"
...
def append_csv_row(
    ...
    if writer is None:
        return
    writer.writerow([*sample, state, f"{prob:.6f}", f"{cpm:.3f}", true_label])
...
def main() -> None:
    ...
    parser.add_argument("--counter-channel", default="gy", help="auto, gyro_mag, gz, gy, gx, acc_mag, ax, ay, az")
    ...
    parser.add_argument("--signal-min-peaks", type=int, default=2)
    parser.add_argument("--signal-min-cpm", type=float, default=30.0)
    parser.add_argument("--signal-max-cpm", type=float, default=160.0)
    parser.add_argument("--signal-min-channel-std", type=float, default=0.04, help="Minimum std of the selected counter channel.")
    ...
    if args.save_csv:
        ...
        csv_writer.writerow(
            ["time_ms", "ax", "ay", "az", "gx", "gy", "gz", "state", "chewing_prob", "cpm", "true_label"]
        )
    ...
    def infer_signal(window_df: pd.DataFrame) -> tuple[float, float, str, int, str]:
        ...
        signal_prob = 1.0 if is_chewing else min(score, non_chewing_cap)
        return signal_prob, cpm, channel_name, count.chewing_count, count.rate_label
    ...
    def infer_once() -> None:
        ...
        print(
            ...
            f"CPM={cpm:5.1f} count={chew_count:2d} channel={channel_name} {rate_label}"
        )

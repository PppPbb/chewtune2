...
def read_dual_csv(path: Path) -> pd.DataFrame:
    ...
    label_col = next((col for col in ("activity_label", "true_label", "label", "state") if col in df.columns), None)
    if label_col is None:
        out["label"] = infer_label(path, df)
    else:
        out["label"] = df.loc[out.index, label_col].map(normalize_label)
...
def iter_windows(
    ...
    for _, group in df.groupby("source_file", sort=False):
        ...
        for start in range(0, len(group) - window_samples + 1, step_samples):
            ...
            if label_counts.empty or float(label_counts.iloc[0]) < min_label_fraction:
                continue
            window = window.copy()
            window["label"] = str(label_counts.index[0])
            yield window

...
```powershell
cd C:\Users\YUN\Desktop\chewtune\chewtune2\S1_chewing_music_system
...
## S1 Music Intervention

S1 keeps the C3 detection method and adds layered music intervention.

Put loop files in:

```text
C:\Users\YUN\Desktop\chewtune\chewtune2\S1_chewing_music_system\music
```
...
```powershell
cd C:\Users\YUN\Desktop\chewtune\chewtune2\S1_chewing_music_system
python python\s1_music_intervention.py --port COM4
```
...
```powershell
python python\s1_music_intervention.py --port COM4 --no-gui
```
...
Fast chewing uses the threshold `100 CPM`: in the latest 4 valid chewing intervals, if at least 2 instantaneous CPM values are above the threshold, S1 switches to `fast`.
